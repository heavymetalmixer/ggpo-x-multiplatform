...
#include <stdexcept> //for throwing cross-platform exceptions
...

{
   ...
   #else
   GGPOSessionCallbacks  callbacks = {nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr};
   Sync::Config config;
...
void Peer2PeerBackend::CheckDesync()
{
        ...
        for (const auto& pair : ep._remoteCheckSums)
        {
            ...
            if (_confirmedCheckSums.count(checkSumFrame))
            {
                ...
                if (remoteChecksum != localChecksum)
                {
                    ...
                    #else
                    //unix or mingw
                    sprintf (buf, "DESYNC Checksum frame %d, local: %d, remote %d, size of checksum maps: %d,%d", checkSumFrame, localChecksum, remoteChecksum, (int)_confirmedCheckSums.size(), (int)ep._remoteCheckSums.size());
                    #endif
                ...
                if (checkSumFrame % 100 == 0)
                {
                    ...
                    #else
                    //unix or mingw
                    sprintf(buf, "Checksum frame %d, local: %d, remote %d, size of checksum maps: %d,%d\n", checkSumFrame, localChecksum, remoteChecksum, (int)_confirmedCheckSums.size(), (int)ep._remoteCheckSums.size());
                    #endif
                    //OutputDebugStringA(buf);
                }
            }
    ...
    for (auto k : keysToRemove)
    {
      ...
      #else
      //unix or mingw
      sprintf(buf, "Erase checksums for frame %d\n",k);
      #endif
      // OutputDebugStringA(buf);
   ...
   if (input.frame != GameInput::NullFrame) { // xxx: <- comment why this is the case
      ...
      // gets incorporated into the next packet we send.
       ...
       if (_confirmedCheckSumFrame >= 0) {
           ...
           #else
           //unix, mingw
           sprintf(buf, "Frame %d: Send checksum for frame %d, val %d\n", input.frame, _confirmedCheckSumFrame, input.checksum);
           #endif
           //OutputDebugStringA(buf);
       }
...
Peer2PeerBackend::IncrementFrame(uint16_t checksum1)
{
    ...
    if (_pendingCheckSums.count(_sync.GetFrameCount()))
    {
        ...
        #if (defined(__MINGW__) || defined(__MINGW32__)) || !defined(_WIN32)
            maxDif = std::max(maxDif, diff);
        #else
        ...
        #else
        //unix, mingw
        sprintf (buf, "Replace local checksum for frame %d: %d with %d, newest frame is %d, max diff %d\n", _sync.GetFrameCount(), oldChecksum, _pendingCheckSums[_sync.GetFrameCount()], max, maxDif);
        #endif
        ...
        if (currentFrame <= _confirmedCheckSumFrame)
        {
            ...
            #else
            //unix
            sprintf(buf, "Changing frame %d in a rollback, but we've already sent frame %d\n", currentFrame, _confirmedCheckSumFrame);
            #endif
        ...
        if (diff >= (_sync.MaxPredictionFrames())) {
            ...
            #else
            //unix, mingw
            sprintf(buf, "diff is bigger than max prediction\n");
            #endif
    ...
    else
    {
      ...
      #else
      //unix, mingw
      sprintf(buf, "Added local checksum for frame %d: %d\n", _sync.GetFrameCount(), cSum);
      #endif
      //OutputDebugStringA(buf);
    }
   ...
   case UdpProtocol::Event::Type::Input:
         ...
         //
            ...
            if (checkSumFrame %120==0)
            {
                ...
                #else
                //unix or mingw
                sprintf(buf, "Received checksum for frame %d, remote cs is %d\n", checkSumFrame, remoteChecksum);
                #endif
                //OutputDebugStringA(buf);
            }
         }
